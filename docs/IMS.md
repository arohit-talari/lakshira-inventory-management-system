...
The master sheet built in the data-foundation stage added real structure: standardized category and supplier reference tables, validated pick-lists for sales channel and status, conditional formatting, and formula-driven derived columns.

But testing it directly, a staging copy handed to the client to enter data the way they actually would, surfaced two gaps that structure alone couldn't close. Business-logic errors, like a selling price coming in under cost, weren't caught by anything in real time: they'd eventually surface in the derived margin columns, but only after the fact, and catching them meant personally reviewing those entries line by line before anything was transferred into the real master sheet. Separately, supplier, weave type, and category were never validated fields at all, so the same supplier ended up entered multiple different ways over time (variant spellings, inconsistent spacing), quietly fragmenting one supplier into several in the data.

The IMS closes both gaps directly: it hardcodes the reference tables into real pick-lists, so a supplier, weave type, or category only ever gets entered one way, while still allowing new ones to be added when they're genuinely new, and it checks business-logic rules in real time, before the next prompt, instead of relying on someone who already knows what to look for to catch them later. It's also a lightweight, purpose-built alternative to a commercial Enterprise Resource Planning (ERP) / Customer Relationship Management (CRM) system, and it had to stay lightweight: the owner was already juggling supplier relationships, sales, marketing, and building a storefront from scratch, so running this system couldn't become its own full-time job. Most small businesses reach for paid software: QuickBooks, Zoho, or an integrated ERP/CRM tied to their Shopify storefront; the IMS does the equivalent (inventory lifecycle, end-to-end sales workflow, customer insights, financial reporting) shaped around how this specific business operates, at a fraction of the integration cost.
