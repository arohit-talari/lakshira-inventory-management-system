...
That said, not every object on the dashboard should respond to a filter. The KPI cards at the top of each view exist to answer a single question: how is the business doing? If those numbers shifted with whatever a viewer happened to have filtered elsewhere on the page, they'd stop serving as a stable, trustworthy baseline, the one thing they're meant to protect. So the dashboard draws a deliberate line: headline KPIs stay immune to filters, while the supporting charts and tables beneath them (Revenue by Customer, the Collection Sell-Through chart, the Recent Transactions table) are explorable and filter-responsive, because their job is to let a user drill in. The rule of thumb: filters belong wherever exploration adds value, and are deliberately excluded wherever they'd compromise a KPI meant to hold one fixed, reliable value irrespective of what else is filtered on the page.

## 4. The Three Views in Detail

### 4.1 Business Overview

This view establishes lifetime health first, then asks whether the business is currently trending in the right direction. Five headline KPIs (Total Revenue, Gross Profit, Gross Margin, Units Sold, and Average Order Value) each report a lifetime total alongside a delta badge comparing the current 30-day window against the prior 30-day window. The delta is the more actionable number: it tells the owner whether recent conditions are trending up or down so they can start investigating the cause while it's still recent, rather than discovering a decline months later in a scheduled report. The **Revenue and Gross Profit Trend** chart underneath complements those two deltas specifically, showing the shape of the whole trajectory behind each, not just a two-point comparison.

Two supplementary objects add depth without diluting the headline KPIs. **Sales Consistency** answers a question a simple revenue trend can't: not whether revenue is moving up or down, but how erratic the swings are. It compares the swing between a period's best and worst month, relative to a typical month, across the last six months against the prior six, a custom metric built specifically because volatility itself is information a business owner needs. A business that swings wildly month to month carries a different kind of risk than one growing steadily, even at the same average growth rate. **Recent Transactions** is a rolling 30-day window of individual closed sales, giving her a ground-level view underneath the aggregate KPIs.

### 4.2 Inventory & Operations

Business Overview measures health; this view identifies what's actively hampering it. Five KPIs, framed around capital at risk rather than abstract inventory statistics: Unsold Inventory (capital currently tied up in stock that hasn't sold), Dead Stock Units and their Cash Exposure (units aged 180+ days without moving, the more painful inflection point since these are functionally stuck), and Units Selling Below Cost with their Cash Exposure (units priced under what they cost to acquire, a guaranteed loss if sold at that price, not just a smaller profit).
