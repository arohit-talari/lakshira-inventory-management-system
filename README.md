...
Inventory was fragmented across 12 separate spreadsheets with no single source of truth, no standardized categories or suppliers, and no consistent pricing method. The first build stage replaced that entirely with one unified master sheet: standardized category and supplier reference tables, data validation on every controlled field, conditional formatting, and formula-driven derived columns. Where there had only been scattered manual tracking, there was now a real data model.
...
A unified sheet still couldn't answer ad hoc business questions at scale, so a Python ETL pipeline extracts data from Google Sheets via the Sheets API, transforms it with pandas, and loads it into a normalized MySQL schema hosted on AWS RDS. The business's live operational data became something queryable for analysis the spreadsheet alone couldn't support.
